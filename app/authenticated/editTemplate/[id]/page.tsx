...
// Add function to fetch template data
...
export default function EditTemplate({ params }: { params: Promise<{ id: string }> }) {
      ...
      if (id) {
        ...
        if (templateData) {
          ...
          if (templateData.categories) {
            ...
            // Initialize edited elements
            const newEditedElements: Record<number, Record<number, Element>> = {};
            templateData.categories.forEach((category: Category) => {
              newEditedElements[category.id] = category.elements.reduce(
                (acc, element) => ({
                  ...acc,
                  [element.id]: element,
                }),
          ...
          // Set variables based on their type
          if (templateData.variables) {
            const linearFeet = templateData.variables.filter((v: Variable) => v.category === "Linear Feet");
            const squareFeet = templateData.variables.filter((v: Variable) => v.category === "Square Feet");
            const cubicFeet = templateData.variables.filter((v: Variable) => v.category === "Cubic Feet");
            const count = templateData.variables.filter((v: Variable) => v.category === "Count");
  ...
  const handleSubmit = async () => {
    try {
      // Determine whether to create new or update existing template
      const method = id ? "PUT" : "POST";
      const url = id ? `${API_URL}/templates/${id}` : `${API_URL}/templates`;
      
      // Convert editedElements to format expected by the server
      interface ElementsMap {
        [key: string]: Record<number, Element>;
      }
      
      const convertedEditedElements = Object.entries(editedElements).reduce<ElementsMap>(
        (acc, [categoryId, elements]) => {
          acc[categoryId] = elements;
          return acc;
        },
        {}
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          // Include edited elements
          updated_elements: convertedEditedElements
        }),
      });
  
      if (!response.ok) {
        throw new Error(`Failed to ${id ? 'update' : 'save'} template`);
      }
  
      const data = await response.json();
      console.log(`Template ${id ? 'updated' : 'saved'} successfully:`, data);
      // Redirect to templates list after successful save/update
      window.location.href = "/authenticated/template";
    } catch (error) {
      console.error(`Error ${id ? 'updating' : 'saving'} template:`, error);
          ...
          <PopoverContent className="w-full p-0">
            <Command>
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {filteredVariables
                    .filter(v => !selectedVariables.some(sv => sv.id === v.id))
                    .map((variable) => (
                      <CommandItem
                        key={variable.id}
                        value={variable.name}
